...
const DEFAULT_PROFILE={businessName:"",budget:0,age:0,income:0,expenses:0,reserve:0,market:"Bangladesh",city:"",occupation:"",experience:"Beginner",skills:"",workStyle:"Builder",category:"Open to anything",goal:"Profit + growth",risk:"Balanced",riskCapacity:"Medium",lossTolerance:0,time:0,targetIncome:0,model:"Open to recommendations"};
...
function openWizard(){
 ...
 window.captureWizard=()=>{["businessName","budget","age","income","expenses","reserve","market","city","occupation","experience","skills","workStyle","category","goal","risk","riskCapacity","lossTolerance","time","targetIncome","model"].forEach(k=>{let e=document.getElementById("w"+k);if(e)d[k]=e.type==="number"?Number(e.value):e.value})};
 ...
 window.finishWizard=()=>{captureWizard();if(!validateWizard(5))return;const reality=forgeRealityCheck(d);state.profile={...d,reality};state.business={name:String(d.businessName||"").trim()||"Your Business",budget:Math.max(0,Number(d.budget||0)-Number(d.reserve||0)),market:d.market,category:d.category,age:d.age,income:d.income,goal:d.goal,risk:d.risk,time:d.time,targetMargin:categoryMargin(d.category),planMode:planMode(d),realityMode:reality.mode,realitySeverity:reality.severity,realityFlags:reality.flags};state.profileLocked=true;state.page="ideas";persistProfile();closeModal();render();toast("Profile locked — Forge is now personalized to you")};
...
function wizardStep(s,d){
 ...
 if(s===4)return "<h3>4 · What should Forge build around?</h3><div class='grid'><div class='span12 field'><label>Business name</label><input class='input' id='wbusinessName' value='"+(d.businessName||"")+"' placeholder='e.g. BlackBole'><div class='small' style='margin-top:5px'>Use your real business name. If you leave this blank, Forge will show ‘Your Business’ instead of inventing a brand name.</div></div><div class='span6 field'><label>Preferred category</label><select class='input' id='wcategory'><option>Open to anything</option><option>Fashion & accessories</option><option>Beauty & personal care</option><option>Home & living</option><option>Electronics</option><option>Food & beverage</option><option>Sports & fitness</option><option>Services / digital</option></select></div><div class='span6 field'><label>Primary goal</label><select class='input' id='wgoal'><option>Profit + growth</option><option>Low-risk side income</option><option>Long-term brand</option><option>Fastest revenue</option><option>Replace my salary</option></select></div><div class='span6 field'><label>Preferred business model</label><select class='input' id='wmodel'><option>Open to recommendations</option><option>Online product selling</option><option>Private label</option><option>Reselling</option><option>Service business</option><option>Wholesale / distribution</option></select></div><div class='span6 field'><label>Target additional monthly income</label><input class='input' id='wtargetIncome' type='number' min='0' value='"+(d.targetIncome||"")+"' placeholder='e.g. 50000'></div><div class='span6 field'><label>Primary market</label><select class='input' id='wmarket'><option>Bangladesh</option><option>India</option><option>UAE</option><option>Saudi Arabia</option><option>USA</option></select></div></div>";
...
function businessName(c){return "Your Business"}
function closeModal(){
...
function render(){
  ...
  try{
    ...
    if(state.page==="plan"&&state.planStatus==="preparing")setTimeout(()=>{if(state.page==="plan")render()},500);
  }catch(err){    console.error("Forge render error",state.page,err);
    const detail=esc(String(err&&err.message||err));
...
(function(){
  ...
  window.forgeSetHeroProduct=function(index){
    ...
    const ids=products.map(function(x){return x.id});
    const q=state.sourcingQueue||[];
    q.forEach(function(x){if(ids.indexOf(x.id)>=0)x.planSelected=true});
